...
import {
  ...
  PaymentFilterParams,
} from "../_queries/build-payment-filters";

export async function downloadFilteredPayments(
  filterParams: PaymentFilterParams = {}
): Promise<{ success: boolean; message: string; data?: string }> {
  try {
    const whereClause = buildPaymentWhereClause(filterParams);

    // 필터링된 결제 데이터 조회 (페이지네이션 없이 전체)
    ...
    console.error("[DOWNLOAD_FILTERED_PAYMENTS_ERROR]", error);
    return {
      success: false,
