...
import { z } from 'zod';
import { refundCashPayment } from '../_actions/refund-action-input';
...
export function CashRefundAction({ row }: RefundActionProps) {
    const payment = row.original;
    const router = useRouter();
    const [open, setOpen] = useState(false);
    const isVirtualAccount = payment.paymentMethod === 'VIRTUAL_ACCOUNT';
    const isWaitingForDeposit = isVirtualAccount && payment.paymentStatus === 'WAITING_FOR_DEPOSIT';
    ...
    const onSubmit = async (values: RefundFormData) => {
        try {
            const result = await refundCashPayment({
