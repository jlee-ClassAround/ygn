...
export function usePaymentsWindow({
    ...
    async function requestPayment() {
        ...
        const commonPayload = {
            ...
            orderId: tossOrderId,
            orderName,
            successUrl: window.location.origin + `/partial-checkout/payment/success?${qs}`,
            failUrl: window.location.origin + '/partial-checkout/payment/fail',
            customerEmail,
            customerName,
            customerMobilePhone,
            metadata: {
    ...
    const handlePurchase = async () => {
        try {
            ...
            if (isPartialPayment) {
                ...
                if (!confirm('해당 상품은 이미 분할결제 중입니다. 이어서 결제하시겠습니까?'))
                    return;
