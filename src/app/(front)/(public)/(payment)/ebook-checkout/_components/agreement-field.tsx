    ...
    <div>
      <div className="flex items-center space-x-2 cursor-pointer">
        <Checkbox
          ...
          type="button"
        />
        <Label htmlFor="all" className="font-semibold text-lg">
          모든 약관에 동의합니다
        </Label>
        <ChevronDown
          className={cn(
            "size-4 !ml-5 transition-transform duration-300",
      ...
      <div
        className={cn(
          "space-y-2 pt-2 pl-5 overflow-hidden transition-all duration-500 ease-in-out",
      ...
      >
        {agreements.map((agreement) => (
          <div key={agreement.id} className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor={agreement.id} className="flex items-center gap-1">
              {agreement.label}
              <Link
                href={agreement.url}
                target="_blank"
